...
// NOTICE (repo root) requires THIRD-PARTY-NOTICES.txt to ship with the
// installer. It is generated by tools/gen-third-party-notices.mjs (the shell
// `notices` script, run by every dist:* script). electron-builder only WARNS on
// a missing extraResources source and still exits 0, so without this the
// installer would silently ship without the required notices file. Fail loudly.
function assertThirdPartyNoticesPresent() {
  const notices = join(__dirname, 'build/THIRD-PARTY-NOTICES.txt')
  if (!existsSync(notices)) {
    throw new Error(
      'build/THIRD-PARTY-NOTICES.txt is missing. Run "pnpm --filter @genoffice/shell run notices" ' +
        '(tools/gen-third-party-notices.mjs) before packaging; the dist:* scripts do this ' +
        'automatically.',
    )
  }
}
...
const config = {
  ...
  extraResources: [
    ...
    // mac.extraResources / win.extraResources so a Linux package does not warn
    // about (and never ships) helpers for other operating systems.
  ],
  ...
  mac: {
    ...
    extraResources: [
      ...
      // macOS-only scanned-page OCR helper (Vision.framework); the engine
      // resolver degrades to the bitmap fallback when absent.
      {
  ...
  win: {
    ...
    extraResources: [
      ...
      // Windows-only scanned-page OCR helper (Windows.Media.Ocr); the engine
      // resolver degrades to the bitmap fallback when absent.
      {
  ...
  linux: {
    ...
    description:
      'Redrob Office is an on-device, offline-first office suite: Redrob Docs, ' +
      'Sheets, Slides, Markdown, PDF, and Hangul editors in one window, with the ' +
      'Redrob AI assistant built in.',
    category: 'Office',
    // Icon SET directory, not the single 1024px png: electron-builder does
    // not resize a lone png, so deb/rpm would install only
    // hicolor/1024x1024/apps/redrob.png — a size absent from the hicolor
  ...
  // packageName pins the control Package field to a stable "redrob" so apt
  // treats successive releases as upgrades of the same package (a different
  // Package name would be seen as an unrelated install). Without it, fpm would
  // derive the name from the "Redrob Office" productName.
  deb: {
